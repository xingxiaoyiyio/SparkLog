...
export const ChatMessage: React.FC<ChatMessageProps> = ({ message, onDelete }) => {
  ...
  return (
    <div className={`flex w-full ${isUser ? 'justify-end' : 'justify-start'} mb-6 animate-fade-in-up group relative`}>
      <div className={`flex max-w-[90%] md:max-w-[75%] gap-3 ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
        
        {/* Avatar */}
        <div className={`flex-shrink-0 w-8 h-8 md:w-10 md:h-10 rounded-xl flex items-center justify-center shadow-sm border ${
          isUser 
        ...
        <button
            ...
            className={`opacity-0 group-hover:opacity-100 transition-opacity absolute top-2 ${isUser ? '-left-8' : '-right-8'} p-1.5 text-slate-300 hover:text-red-400 rounded-full hover:bg-red-50`}
            title="删除此条"
        >
            <Trash2 size={14} />
        </button>
