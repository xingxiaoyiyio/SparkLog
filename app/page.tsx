...
export default function Home() {
  ...
  const handleSendMessage = async (text: string, imageBase64?: string) => {
    ...
    try {
      if (text.toLowerCase().includes("wrap up") || text.includes("日结") || text.includes("今日总结")) {
    ...
    if (!hasUserContent) {
      if (!isAutoTrigger) {
        const noRecordMsg = {
          id: Date.now().toString(),
          role: Role.MODEL,
          text: "还没有记录任何碎片呢！先聊点什么吧，比如发张照片或者分享个链接？🤔",
          timestamp: new Date()
        };
    ...
    try {
      ...
      if (isAutoTrigger) {
        setMessages(prev => [...prev, {
          id: Date.now().toString(),
          role: Role.MODEL,
          text: "✨ 这一天结束啦，我已经自动帮你生成了今日日结，快去历史记录里看看吧！晚安 🌙",
          timestamp: new Date()
        }]);
      }
